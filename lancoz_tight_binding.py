"""
Penrose Lattice - Manual Lanczos (Dense Matrix + Explicit Inverse)
...
class PenroseLanczosDense:
    """
    Implementasi Lanczos dengan pendekatan 'Brute Force' Dense Matrix.
    Menggunakan np.linalg.inv untuk Shift-Invert.
    """
    ...
    def build_hamiltonian(self):
        """
        Membangun Hamiltonian sebagai DENSE Matrix (numpy array biasa).
        Peringatan: Memakan memori N^2.
        """
        print(f"[2] Membangun Matriks Hamiltonian DENSE...")
        
        # Inisialisasi matriks penuh nol
        self.hamiltonian = np.zeros((self.N, self.N))
        
        # Isi elemen (Hopping)
        for i, j in self.raw_edges:
            self.hamiltonian[i, j] = -self.t
            self.hamiltonian[j, i] = -self.t
            
        print(f"    ✓ Matriks Dense {self.N}x{self.N} siap.")
        # Cek ukuran memori
        mem_size = self.hamiltonian.nbytes / (1024**2)
        print(f"    ℹ️  Ukuran Memori: {mem_size:.2f} MB")

    def run_lanczos_shift_invert(self, k_steps=50, sigma=1e-9):
        """
        Algoritma Lanczos dengan EXPLICIT INVERSE (np.linalg.inv).
        """
        print(f"\n[3] Menghitung Invers Matriks Secara Eksplisit...")
        t0 = time.time()
        ...
        # 2. Hitung Invers (O(N^3)) - Bagian Berat!
        #    A = (H - sigma*I)^-1
        try:
            M_inv = np.linalg.inv(M)
        except np.linalg.LinAlgError:
            print("    ❌ Matriks Singular! Coba ubah nilai sigma sedikit.")
            exit()
            
        print(f"    ✓ Invers selesai dalam {time.time()-t0:.4f} detik.")
        ...
        for j in range(k_steps):
            # --- PERUBAHAN UTAMA ---
            # w = A * v (Perkalian Matriks Biasa)
            # Karena kita punya M_inv, kita kalikan langsung.
            w = np.dot(M_inv, v_curr)
    ...
    def plot_result(self, filename='imgs/dense_lanczos_result.png'):
        """Visualisasi Identik"""
...
def main():
    print("="*60)
    print("   DENSE MATRIX LANCZOS (EXPLICIT INVERSE)")
    print("="*60)
    
    solver = PenroseLanczosDense()
    solver.load_data('data/penrose_lattice_data.npz')
    solver.build_hamiltonian()
    
    # Jalankan solver
    # Kita coba 3 iterasi saja dulu (sesuai kode sebelumnya)
    # Tapi idealnya butuh lebih banyak (misal 20-50) untuk konvergensi bagus
