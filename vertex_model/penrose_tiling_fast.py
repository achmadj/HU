...
def main():
    ...
    parser.add_argument('--iteration', type=int, default=4, 
                        help='Maximum iteration number (default: 4)')
    parser.add_argument('--save-all', action='store_true', 
                        help='Save data for each iteration separately')
    ...
    print(f"Maximum iteration: {args.iteration}")
    print_separator()
    ...
    # Save iteration 0 if --save-all flag is set
    if args.save_all:
        ...
        npz_file_iter = 'vertex_model/data/penrose_lattice_iter0.npz'
        np.savez(npz_file_iter,
                 ...
                 iteration=0,
                 phi=penrose.phi)
        print(f"  💾 Saved iteration 0: {npz_file_iter}")
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        # Save this iteration if --save-all flag is set
        if args.save_all:
            vertex_ids = np.arange(len(penrose.vertex_positions), dtype=np.int32)
            vertex_coords = np.array(penrose.vertex_positions, dtype=np.float64)
            edge_list = np.column_stack([penrose.edge_sources, penrose.edge_targets]).astype(np.int32)
            ...
            npz_file_iter = f'vertex_model/data/penrose_lattice_iter{iteration}.npz'
            np.savez(npz_file_iter,
                     vertex_ids=vertex_ids,
                     vertex_coords=vertex_coords,
                     edge_list=edge_list,
                     ...
                     iteration=iteration,
                     phi=penrose.phi)
            print(f"  💾 Saved iteration {iteration}: {npz_file_iter}")
    ...
    # Save as pickle
    pickle_file = 'vertex_model/data/penrose_lattice_data.pkl'
    with open(pickle_file, 'wb') as f:
        pickle.dump(data_to_save, f)
    print(f"  ✓ Saved to pickle: {pickle_file}")
    
    # Save as numpy arrays
    npz_file = 'vertex_model/data/penrose_lattice_data.npz'
    ...
    if args.plot:
        ...
        plt.tight_layout()
        plt.savefig('vertex_model/imgs/penrose_all_iterations_fast.png', dpi=200, bbox_inches='tight')
        print("  ✓ Saved plot: vertex_model/imgs/penrose_all_iterations_fast.png")
        ...
        plt.tight_layout()
        plt.savefig('vertex_model/imgs/penrose_iter0_iter1_overlapped_fast.png', dpi=200, bbox_inches='tight')
        print("  ✓ Saved plot: vertex_model/imgs/penrose_iter0_iter1_overlapped_fast.png")
